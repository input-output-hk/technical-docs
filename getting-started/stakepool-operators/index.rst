...
* A block-producing node will be configured with various key-pairs and an operational certificate needed for block generation (cold keys, KES hot keys and VRF hot keys). It only connects to its relay nodes.

* A relay node will not be in possession of any keys and will therefore be unable to produce blocks. It is connected to its core node, other relays and external nodes.
